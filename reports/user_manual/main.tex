...
\vfill
Private source repository: shared with the teaching team separately\\
...
\path{course_models/} & Optional embedded course model directory. The large \texttt{.pt} files are ignored by Git but can be included in the final zip.\\
\bottomrule
\end{tabularx}
\normalsize

\section{Bundling the Course Model}
The supplied course model can be bundled inside the project under \path{course_models/}. This makes a zip package self-contained for offline model inference, while keeping the Git repository small and safe to push.

\begin{manualbox}{Required Embedded Model Files}
\begin{verbatim}
...
Prepare the folder from the course asset directory:
...
The package script creates \path{dist/AussieEcoLens_submission.zip}. If \path{course_models/model.pt} and \path{course_models/mdv5a.pt} exist locally, they are copied into the zip even though they are ignored by Git.
